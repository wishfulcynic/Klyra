...
import { Copy, ExternalLink, ChevronDown, Menu, X, Sun, Moon, WalletIcon, AlertTriangle } from "lucide-react"
import { useTheme } from "@/components/theme-provider"
import Link from "next/link"
import { motion } from "framer-motion"
import { SUPPORTED_CHAINS } from "@/lib/contract"

export function Header() {
  const { address, connect, disconnect, isConnecting, isConnected, chainId } = useWallet()
  ...
  const isWrongNetwork = isConnected && chainId && chainId !== SUPPORTED_CHAINS.BASE_MAINNET;
  ...
  return (
    <>
            ...
            {isConnected && address ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
                    className={`flex items-center gap-2 bg-white border-gray-200 hover:bg-gray-50 text-gray-900 rounded-full shadow-sm transition-all duration-200 ${isWrongNetwork ? '!border-red-500 !text-red-600' : ''}`}
                  >
                    <div className={`${isWrongNetwork ? 'bg-red-500' : 'bg-green-500'} w-2 h-2 rounded-full`}></div>
      ...
      {isWrongNetwork && (
        <div className="bg-red-100 border-b border-red-300 text-red-700 px-4 py-2 text-sm text-center font-medium">
          <div className="container max-w-7xl mx-auto flex items-center justify-center gap-2">
             <AlertTriangle className="h-4 w-4 shrink-0" />
             <span>Wallet connected to wrong network. Please switch to Base Mainnet.</span>
           </div>
        </div>
      )}
      ...
      {mobileMenuOpen && (
        <motion.div
          ...
          transition={{ duration: 0.3 }}
          className="md:hidden bg-white border-t border-gray-200 py-4 absolute top-16 left-0 right-0 z-40 shadow-md"
        >
